...
TEST_F(PFData_test, MultiplePFB){

    char filename1[2048];

    getcwd(filename1, 2048);

    auto t_start = std::chrono::high_resolution_clock::now();

    strcat(filename1,"/forcing/NLDAS.APCP.000025_to_000048.pfb");
    PFData test1(filename1);
    int retval = test1.loadHeader();
    retval = test1.loadClipOfData(2806, 1060, 10 ,16);
    test1.close();
    auto t_end = std::chrono::high_resolution_clock::now();

    std::cerr << "[          ] time taken = " << std::chrono::duration<double, std::milli>(t_end-t_start).count() << std::endl;
    ASSERT_EQ(0, 0);

}

TEST_F(PFData_test, CalcAvg) {
    // this file needs to exist, and should load
    PFData test_in("tests/inputs/press.init.pfb");
    int retval = test_in.loadHeader();
    ASSERT_EQ(0,retval);
    retval = test_in.loadData();
    ASSERT_EQ(0,retval);
    double sum = 0;
    ...
    test.setDX(1.0);
    test.setNZ(1);
    test.setNY(test_in.getNY());
    test.setNX(test_in.getNX());

    double mean[ test_in.getNY()* test.getNX()];

    for(int y = 0; y < test_in.getNY(); ++y){
        //sum=0;
        for(int x = 0; x < test_in.getNX(); ++x){
            sum = 0;
            for(int z = 0; z < test_in.getNZ(); ++z){
                 //std::cout << "test " << test(z, y, x)<<'\n';
                 sum += test_in(z, y, x);
            }
            std::cout << y <<" , "<<x<< " , "<< x+y*test_in.getNX() <<std::endl;
            mean[(x+y*test_in.getNX())] = sum/test_in.getNZ();
          // std::cout << "The mean is " <<sum/(test_in.getNZ())  <<std::endl;
         //  std::cout<< "------------------------------------"<<std::endl;
        }
