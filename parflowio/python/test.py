...
class PFDataClassTests(unittest.TestCase):
    ...
    def test_print_data(self):
        ...
        with np.printoptions(threshold=np.inf):
            print(mean_data)
        print("the dimension of mean is\n")
        print(mean_data.shape)

    def test_read_xarray(self):
        ds = xr.open_dataset('press.init.pfb', name='var')
        print('test')
        print(ds.values)
        print('---------------')
    ...
    def test_read_xarray_output(self):
        ds = xr.open_dataset('test_write_file_out.pfb', name='var')
        print('hello_rest')
        print(ds.values)
        print('xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
        #ds_agg = ds.mean(dim='z')
        ds_numpy = ds['var'].to_numpy()
        with np.printoptions(threshold=np.inf):
            print(ds_numpy)
